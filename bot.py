...
class SqueezeMomentumIndicator:
    ...
    def generate_signals(self, df):
        ...
        # Force strict boolean type and handle NaN safely without using ~
        squeeze_on_safe = data['squeeze_on'].fillna(False).astype(bool)
        mom_inc_safe = data['momentum_increasing'].fillna(False).astype(bool)
        
        # Detect Squeeze Release (Was True in previous candle, and False now)
...
def main():
    print("=== Running Updated Bot Version 2.0 ===") # This line proves the new code is running
    if not TELEGRAM_TOKEN or not CHANNEL_ID:
