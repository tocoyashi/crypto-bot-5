...
class SqueezeMomentumIndicator:
    ...
    def generate_signals(self, df):
        ...
        # FIX: Safely convert to boolean to handle NaN values at the start of the data
        squeeze_on_safe = data['squeeze_on'].fillna(False).astype(bool)
        data['squeeze_release'] = squeeze_on_safe.shift(1) & ~squeeze_on_safe
        
        # FIX: Safely handle momentum_increasing for NaN values
        mom_inc_safe = data['momentum_increasing'].fillna(False)
        
        # Buy Condition
        buy_cond = (data['squeeze_release']) & (data['momentum'] > 0) & (mom_inc_safe)
        
        # Sell Condition (Two bar rule & Cross zero)
        sell_cond = (
            (data['momentum'] < 0) & (data['momentum'].shift(1) >= 0) |
            ((~mom_inc_safe) & (~mom_inc_safe.shift(1)) & (data['momentum'] > 0))
        )
