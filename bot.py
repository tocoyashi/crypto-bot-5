...
TIMEFRAME = '15m'      
TOP_N_COINS = 50       
# Stablecoins to ignore (they don't squeeze)
    ...
    try:
        ...
        for symbol, ticker in tickers.items():
            if symbol.endswith('/USDT') and symbol not in STABLECOINS:
                ...
                if vol > 1000000: # Only get coins with more than 1M volume to filter junk
...
def main():
    print("=== Running Multi-Coin Squeeze Bot ===")
    if not TELEGRAM_TOKEN or not CHANNEL_ID:
    ...
    # 1. Get the list of top 50 coins
    ...
    for symbol in top_coins:
        try:
            # Pause for half a second to respect MEXC rate limits
            time.sleep(0.5) 
            ...
            # If a coin fails (e.g., delisted or no data), skip it and continue with the rest
            pass 

    print(f"[{datetime.now()}] Scan finished. Total signals found in this round: {signals_found}")
