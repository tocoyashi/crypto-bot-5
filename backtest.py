...
# ================= Backtest Settings =================
TIMEFRAME = '15m'
TOP_N_COINS = 35      # Changed to 35
DAYS_BACK = 90        # Changed to 90 Days (3 Months)
...
TP4_PERC = 7.0     
TRAIL_BE_PERC = 0.25 
MEXC_FEE_PER_TRADE = 0.48 # Mathematically proven fee deduction

STABLECOINS = ['USDC/USDT', 'TUSD/USDT', 'DAI/USDT', 'FDUSD/USDT', 'USDP/USDT', 'PYUSD/USDT']
BLACKLIST = [
    "UPC/USDT", "KAZAR/USDT", "PEPE/USDT", "TOYL/USDT", 
    "STAR/USDT", "RAIN/USDT", "SAFE/USDT", "S/USDT",
    "SHIB/USDT", "FLOKI/USDT", "BONK/USDT", "WIF/USDT", 
    "LUNC/USDT", "BOME/USDT", "NEIRO/USDT"
]
...
def get_top_mexc_coins(limit=35):
...
def run_backtest():
    print(f"=== Running Final 90-Day Backtest (With Real Fees) ===")
    ...
    for symbol in top_coins:
        print(f"Backtesting {symbol} (90 days)...")
        ...
        for sig_time in signal_indices:
            ...
            # DEDUCT MEXC FEES (0.48% per trade cycle)
            trade_pnl -= MEXC_FEE_PER_TRADE
    ...
    msg = f"""📊 <b>FINAL 90-Day Backtest (Real Net Profit)</b> ⏱️ {DAYS_BACK} Days
                         
🔍 <b>Assets:</b> Top {TOP_N_COINS} MEXC Coins (Cleaned)
📈 <b>Timeframe:</b> {TIMEFRAME}
⭐ <b>Leverage:</b> {LEVERAGE}x
💸 <b>MEXC Fees (0.48%):</b> <b>DEDUCTED</b>

━━━━━━━━━━━━━━━━━━━━
📊 <b>Total Signals:</b> {total_trades}

🟢 <b>Full Winners:</b> {net_profit_trades} (<code>{win_rate:.1f}%</code>)
🟡 <b>Trail SL:</b> {total_trail_sl} (<code>{trail_rate:.1f}%</code>)
🔴 <b>Full Losers:</b> {total_sl} (<code>{loss_rate:.1f}%</code>)

━━━━━━━━━━━━━━━━━━━━
💰 <b>Real Net Performance:</b>
• Net Profit: <code>{gross_profit_points:,.0f}</code> pts
• Net Loss: <code>{gross_loss_points:,.0f}</code> pts
• <b>Final Net PnL:</b> <code>{total_pnl_points:,.0f}</code> pts

📈 <b>Real Net Growth / Trade:</b> <code>{expected_capital_growth:.2f}%</code>
{'🟢 <b>VERDICT: PROFITABLE SYSTEM</b>' if total_pnl_points > 0 else '🔴 <b>VERDICT: NEEDS OPTIMIZATION</b>'}
━━━━━━━━━━━━━━━━━━━━"""

    print("Sending Final 90-Day report to Telegram...")
